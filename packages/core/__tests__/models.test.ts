...
describe("Model Provider Configuration", () => {
	...
	describe("Livepeer Provider", () => {
		test("should have correct endpoint configuration", () => {
			expect(models[ModelProviderName.LIVEPEER].endpoint).toBe(
		...
		test("should have correct model mappings", () => {
			...
			expect(livepeerModels[ModelClass.SMALL]).toBe(
				"meta-llama/Meta-Llama-3.1-8B-Instruct",
			);
			expect(livepeerModels[ModelClass.MEDIUM]).toBe(
				"meta-llama/Meta-Llama-3.1-8B-Instruct",
			);
			expect(livepeerModels[ModelClass.LARGE]).toBe(
				"meta-llama/Meta-Llama-3.1-8B-Instruct",
			);
...
describe("Generation with Livepeer", () => {
	test("should have correct image generation settings", () => {
		...
		expect(livepeerConfig.model[ModelClass.IMAGE]).toBe(
	...
	test("should use default image model", () => {
		...
		expect(models[ModelProviderName.LIVEPEER].model[ModelClass.IMAGE]).toBe(
			"ByteDance/SDXL-Lightning",
		);
	});
});
