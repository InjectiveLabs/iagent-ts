...
describe("LensClient", () => {
	...
	describe("authenticate", () => {
		it("should authenticate successfully", async () => {
			...
			expect(
				client["core"].authentication.generateChallenge,
	...
	describe("getPublicationsFor", () => {
		...
		it("should handle fetch errors", async () => {
			vi.mocked(client["core"].publication.fetchAll).mockRejectedValueOnce(
				new Error("Fetch failed"),
			);
			await expect(client.getPublicationsFor("0x123")).rejects.toThrow(
				"Fetch failed",
			);
		});
	});
	...
	describe("getProfile", () => {
		it("should fetch profile successfully", async () => {
			...
			expect(client["core"].profile.fetch).toHaveBeenCalledWith({
				forProfileId: "0x123",
			});
		});

		it("should handle profile fetch errors", async () => {
			vi.mocked(client["core"].profile.fetch).mockRejectedValueOnce(
				new Error("Profile fetch failed"),
			);
			await expect(client.getProfile("0x123")).rejects.toThrow(
